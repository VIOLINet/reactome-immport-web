<template>
  <v-card outlined>
    ...
    <v-card-text>
      ...
      class="mt-5"
      v-if="compareFrom.enrichmentResults.pathways && compareTo.enrichmentResults.pathways"
  ...
  computed: {
    fiNetwork() {
      ...
      const rtn = [];
        ...
        //if no network exists, return empty array
        if((!network1 || network1.legnth === 0) && (!network2 || network2.length === 0))
          return []
