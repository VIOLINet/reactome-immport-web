  ...
  <v-card outlined>
    ...
    <v-expand-transition>
      <v-card-text v-show="show">
        <!-- Somehow need to cast these text into numbers manually -->
        <plot
          :geneExpressionResults="geneExpressionResults"
          :logFDLower="Number(logFCLowerBoundInput)"
          :logFDUpper="Number(logFCUpperBoundInput)"
          :pValueCutoff="Number(pValInput)"
          :adjPValueCutoff="Number(adjustedPValInput)"
        >
        </plot>
        <v-card outlined>
          <v-data-table
...
export default {
  ...
  watch: {
    adjustedPValInput() {
      this.fireCutoffValueChanged();
    },
    pValInput() {
      this.fireCutoffValueChanged();
    },
    logFCLowerBoundInput() {
      this.fireCutoffValueChanged();
    },
    logFCUpperBoundInput() {
      this.fireCutoffValueChanged();
    }
  },

  mounted() {
    // Initialize the cutoff values
    this.fireCutoffValueChanged();
  },
  ...
  methods: {
    fireCutoffValueChanged() {
      this.$emit("cutoffValuesUpdated", {
        pValueCutoff: Number(this.pValInput),
        adjPValueCutoff: Number(this.adjustedPValInput),
        log2fdLower: Number(this.logFCLowerBoundInput),
        log2fdUpper: Number(this.logFCUpperBoundInput),
      });
    },
